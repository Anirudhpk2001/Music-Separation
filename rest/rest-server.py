...
"""
    Configure test vs. production environment.
 """
global redisHost, minioHost, minioUser, minioPasswd, minioClient, queueBucketName, outPutBucketName
...
def separate_music(fileName):
    """
    Route for separating music.
    """
    try:
        # Get the hash of the filename
        hashedMessage = hashlib.md5(fileName.encode()).hexdigest()
        # If a music is being scanned for the first time
        if redisNameToHash.get(fileName) is None:
            content = request.get_json()
            music_data = content['music_data']

            # Add mp3 file to minio bucket
            minioClient.put_object(queueBucketName, fileName, BytesIO(base64.b64decode(music_data)),
                                    length=len(base64.b64decode(music_data)), part_size=10*1024*1024,
                                    content_type='audio/mpeg', progress=Progress())
...
def get_from_queue():
    """
    Route to get items from the queue.
    """
...
def download_tack_file(hashVal, audio):
    """
    Route to download a track file.
    """
...
def delete_from_redis_and_minio(hashVal):
    """
    Route to delete items from Redis and MinIO.
    """
    response = {
        'delete': True,
        'reason': 'Hash Found'
