...
#
REST = os.getenv("REST") or "localhost:8080"
...
def mkReq(reqmethod, endpoint, data, verbose=True):
    ...
    if verbose and data != None:
        ...
        print(f"mp3 is of type {type(data['music_data'])} and length {len(data['music_data'])} ")
...
for mp3 in glob.glob("data/short*mp3"):
    ...
    mkReq(requests.post, "separate/"+mp3,
        data={
            "music_data": base64.b64encode( open(mp3, "rb").read() ).decode('utf-8'),
            "callback": {
    ...
    mkReq(requests.get, "queue", data=None)
